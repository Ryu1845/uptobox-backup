...
r = requests.get('https://uptobox.com/api/user/files?token=' + token + '&orderBy=file_name&dir=asc&offset=0&path=%2F%2F&limit=100')
loaded_json = json.loads(r.text)
page_counter = loaded_json["data"]["pageCount"]

for i in range(page_counter):
    try:
        r2 = requests.get('https://uptobox.com/api/user/files?token=' + token + '&orderBy=file_name&dir=asc&offset=' + str(i * 100) + '&path=%2F%2F&limit=100')
